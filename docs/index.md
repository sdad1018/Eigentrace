...
<div class="wrap">
  ...
  <section>
    ...
    <p>EigenTrace adds a second route to the same reading that does not begin with an instruction: deterministic arithmetic on a frozen embedding space surfaces the buried concepts, inspectably and identically every run. Tested head-to-head, the two routes reach the same depth (insight 3.32 vs 3.35 across 175 summaries scored by 5 judges, 788 parsed scores; the frozen route surfaces 92% of the same concepts a tuned prompt finds). The judges are the same five models that wrote the summaries, so each author's own model is one of its five judges (about a fifth of the scores), and the scores cluster by the 7 stories; no model is the sole judge of its own output, and an ex-self re-score is in progress.</p>
