...
MATH_EXPLAINERS = [
    ...
    ("SVD null space projection", "We stack all five model responses into a matrix and decompose it. The last direction, the one with zero energy, is the null space. That direction represents what no model's summary included. We project it onto the original article to find which specific fact lives in the blind spot."),
...
def generate_script_v3(seg: dict, audit_ctx: dict) -> list[dict]:
    ...
    # ── 3c. SUMMARY PLUS (spicy second pass - all 5, concept-informed) ──
...
# ── 13b. MISTRAL INTERPRETATION + MECHANICAL SWERVE CORRECTION ───
    ...
    try:
        ...
        _sv_cls = _ching_classify(_sv_vec)
        _sv_name = _sv_cls.get("name")

        # PERSIST the signature into the segment so it is queryable downstream
        # (weasel selection, EigenChing page, analysis). Previously computed
        # only for narration and thrown away.
        attr["eigenching"] = {
            "signature": list(_sv_vec),
            "axes": _best6,
            "archetype": _sv_cls.get("archetype_name"),
            "name": _sv_name,
            "tier": _sv_cls.get("tier"),
            "distance": _sv_cls.get("distance"),
        }
