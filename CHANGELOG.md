...
| 2026-03-27 | null_space_claims created (SVD null vector projected onto source claims; no null-space readout existed before) | null_space_claims |
...
| 2026-09-19 | `entity_retention` rule v1 to v2 (entity list filtered; presence is a case-sensitive whole-word test on the family key or a registered alias, not a case-insensitive substring of the whole capitalised run); `source_void.absent_words` no longer counts a UK/US spelling variant present in a summary as a dropped word | entity_retention, entity_abstraction_rate, compression_score, EigenChing axis 4 and every archetype that reads it; absent_words, absent_count, absent_ratio, EigenChing axis 2; docs/soul.md readings; the Summary Plus Channel A word list |

## [2026-09-19]

### Changed

- **EigenChing archetype counts (`docs/eigenching_data.json`, `docs/eigenching.html`, `docs/eigenching_distribution.md`).** Old: `eigenching_report.py` keyed each state beat on the text before its first comma (`text.split("EigenChing state:")[1].split(",")[0]`), since the generator's first version. `eigenching.py:format_broadcast` writes the beat as `"EigenChing state: {name}. {description}"`, and `classify` puts a comma inside `{name}` only for a near-miss (`"The Cornering, verbs recovering"`), never for an exact archetype, so the key returned a bare archetype name only for states one or two axes away from that archetype and returned name-plus-description for an exact hit. Every published count was a near-miss, every exact hit was dropped, and the percentages summed to 69.2% because `total` counted all parsed beats while only the near-misses reached a bucket. New: the beat is keyed on the aired name in full (the text between the prefix and the first period; archetype and morphological names contain no period), matched case-insensitively and whitespace-normalised against the 32 archetype names. `count` and `pct` are exact hits only; near-misses are counted separately in `near_miss_count` / `near_miss_pct` and labelled as near-misses, with their own examples. Pre-EigenChing `"EigenTrace state vector."` beats (359 of 3,817) are now read back from their axis words and classified the same way instead of being ignored. Why: the dashboard's counts were counts of something other than what each card named; see Withdrawal 24. Affects: every archetype count and percentage on `docs/eigenching.html`, the "archetypes observed" and "never observed" totals (22/10 -> 8/24), the card examples (now exact-hit stories), and `total_segments` (3,458 parsed beats -> 3,817 state beats). Numbers stated before this date are not comparable.
- **`docs/eigenching_distribution.md`.** Old: a hand-written 2026-04-16 snapshot of 6,509 segments, 4,543 of which carried no compression or source-void block, whose values `state_vector.extract_signals` read as 0; the quantizer sends 0 to entity -1 and verb_drift +1, so an unmeasured row landed on "names dropped, verbs intact" by construction and the page reported that cell as a finding. New: generated by `eigenching_report.py:write_distribution` on every run. An absent compression / source-void / per-model-VIX block is MISSING, not 0; rows with any MISSING axis are excluded from the state-distribution denominator and their count is shown (at the 2026-09-19 rebuild: 8,042 of 13,467 story rows MISSING, 5,425 measured). The entity/verb cell is reported on measured rows only (871 of 5,425, 16.1%), beside what reading MISSING as 0 would produce (8,913 of 13,467). Affects: the whole page. Its earlier numbers are not comparable with these.

- **`entity_retention`, `entity_abstraction_rate`, `compression_score` (`eigentrace_math.py:score_language_compression`, EigenChing axis 4, `docs/soul.md`, the daily data export).** Old (rule v1, since the layer existed): the entity list was every run of capitalised words in the source plus all-caps tokens, minus 24 sentence starters, and an entity counted as retained when the whole run appeared in the response as a case-insensitive substring. New (rule v2): the same runs minus scraper chrome and runs spanning a line break; leading auxiliaries and trailing verbs trimmed off headline fragments ("Trump Says" to "Trump"); one-token runs that only ever start a sentence and that the source itself also writes in lower case dropped; and an entity counts as retained only when a registered surface form -- the full run, the family/last-token key with particles stripped and generational suffixes dropped, or a registered press alias -- appears as a case-sensitive whole word. Family-first names are keyed on the family token from a registered map, so "Xi Jinping" is keyed Xi, not Jinping. Why: measured on 1,524 five-model body_visible stories (99,075 pairs), 15.3% of the pairs v1 scored retained were substring hits inside another word and 23.1% of the person pairs it scored missing were present; 24.33% of the v1 list was not a named entity of any kind (`analysis/experiments_v1/entity_retention_audit/`). Measured effect on that cell: 0.5468 to 0.5541 overall (the list shrinks 14.2%, 99,075 to 84,995 pairs), 0.5641 to 0.6485 on the spaCy PERSON subset scored over the identical entity set; per-story median 0.556 to 0.562, 5th percentile 0.333 to 0.280, 95th 0.780 to 0.844; axis-4 trit split -1/0/+1 51/855/618 to 98/750/676. The five-model ranking (Grok, DeepSeek, Gemini, ChatGPT, Claude) does not change. Both values are computed on every call and both are stored: `entity_retention_v1`, `entity_retention_v2` and `entity_retention_rule` are in the `compression` block of every segment written from this date, and `entity_rule="v1"` (or `EIGENTRACE_ENTITY_RULE=v1`) reproduces the old number exactly. Numbers across this date are not comparable.
- **`source_void.absent_words`, `absent_count`, `absent_ratio` (`eigentrace_math.py:source_anchored_void`, EigenChing axis 2, the Omission Ledger posts, the daily data export).** Old: a source word counted as absent when neither its surface form nor its Porter stem appeared in any response, excluding headline derivatives. New: a source word whose British/American counterpart spelling a model used is not absent either. Six rule families (-ise/-ize, -our/-or, -re/-er, -ll-/-l-, -ce/-se, -ogue/-og) with an exception list for words that end in -ise in every variety and an ambiguous list held back; the families measured but not covered are -wards, ae/oe and the miscellaneous pairs (programme, grey, judgement). Why: the exact-token test recorded a British spelling as dropped about 90% of the time against about 57% for an American one, and the site's own Summary Plus audit page published `authorised` and `defence` as dropped facts when 22 and 20 of its 25 baselines wrote `authorized` and `defense` (`analysis/experiments_v1/quick_findings/F2_spelling_drops/`). Measured effect on the same 1,524 stories: absent_words 21,717 to 21,483 (234 rescued, 1.08% of drops), mean absent_ratio 0.1845 to 0.1826, median 0.1800 to 0.1765, 190 stories (12.5%) changed, 12 axis-2 trits flip. `absent_count_v1`, `absent_ratio_v1`, `spelling_variant_kept` and `absent_rule` are stored beside the new values. Top rescued pairs: defence/defense 43, centre/center 16, neighbouring/neighboring 16, centres/centers 13, defences/defenses 13.
- **Dropped-actor channel of Summary Plus (`confront_keeper_v3.py:ner_actors_dropped`, fed to the prompt via `confront10_final_BOTH.derive_channels` and `batch_producer.py`).** Old: the actor was keyed on `name.split()[-1].lower()` and matched case-insensitively against the lowercased summaries, so a family-first name was keyed on the given name and a particle was glued into the key. New: `name_key.name_present`, the same rule as entity_retention v2. This is a population change for which words reach the restoration prompt, not a published figure: measured on the 1,090 affected model-rows of the corpus names panel, drops fall 455 to 425 and the published absent-from-every-model stratum moves 646/3,800 to 645/3,800, i.e. 17.0% either way (`analysis/experiments_v1/name_order_audit/`). Channel A also no longer offers a word back when the summaries wrote its other spelling.
- **`docs/soul.md` "Current Instrument Readings".** Old: the table was printed from whatever the last 24 hours held, and when they held nothing `soul_updater.update()` returned before writing, leaving the previous file and its numbers in place under a heading that said "last 24h" -- on 2026-09-16 that page showed "Entity Retention 72% (3)" from three stories measured about 48 hours earlier, the pipeline having produced no scored story since 2026-09-14 18:42. New: below three measured stories the table is replaced by a statement that there is no current reading, with the timestamp of the last measured story; the threshold warnings are suppressed; the page is regenerated even when nothing was measured; and a calibration with no readings is never saved over the last real one. Affects: `docs/soul.md` and anything conditioned on it.

## [2026-09-10]

### Fixed

- **Ticker "Friction" (on-air ticker line).** Old: `segment_player.update_ticker` read a top-level `gap_vix` key that only the pre-batch `proxy_auditor` records carried (segment files dated before 2026-03-24 have it; `batch_producer` has never written it), so from the day the player started reading it (`b64573b6`, 2026-04-21) the ticker printed `Friction: 0.0000` on every story. New: `attribution.mean_vix` (fallback: mean of `attribution.model_vix`), one decimal, the same mean VIX the archive beat and the export carry. Why: the ticker number was not a measurement. Commit `a606d537` (the repo copy of the player; the code comment dates the live-player patch 2026-09-09). Affects: ticker overlay only; stored numbers unchanged.
...
- **Pundit desk** (`pundit_desk.py` added in `9cdab40d`, 07-03; wired into the producer in `1aeda9df`, 07-05): new own-output segment type built from the roundtable record (`pundit_desk.build_record`). In that record (`batch_producer`, roundtable section) a killshot without `omitted_by` is labelled "all five models"; this is prompt text for the pundits, not a stored metric.
...
- **Response length caps removed** (`6cc77198`). Old: prompt "In exactly 2 sentences: what happened and one concrete implication", system prompts "Be direct. 2 sentences.", Anthropic `max_tokens` 350, Mistral 500, `model_responses` truncated at 500 characters, `source_body` stored at 2,000 characters. New: "Explain what happened and the concrete implications", "Be direct and thorough", `max_tokens` 1000 / 2000, no truncation, `source_body` 5,000 characters. Affects `consensus_density` (longer same-prompt responses embed closer), every VIX and `state_flag`, `absent_ratio` (more source words covered), hedge counts (more words), `entity_retention`, `coverage_ratio` and killshots.
...
- `segment_player.py` began reading a top-level `gap_vix` key for the ticker (`b64573b6`) that `batch_producer` segments do not carry (only pre-2026-03-24 `proxy_auditor` records have it): ticker Friction reads 0.0000 from here until the 2026-09-09/10 fix.
...
- `beat_15e_spectral_clusters` on air (`03bed83d`); `docs/spectral_clusters.json` added to the hourly refresh 2026-04-21 (`6047c970`) and regenerated the same day with a boilerplate filter and higher threshold (`35b6d1e9`), a sparsified 5-cluster graph (`c93c31a8`) and PMI-weighted adjacency (`9e9d5ebc`); these three are commits of the JSON file only, described by their commit subjects. The message of `1afd3d9a` (2026-05-13) reports a NameError fix that makes the spectral Laplacian recompute from the filtered co-occurrence matrix, but its only diff is the beat 13 source-recovery rewrite in `script_v3.py`; that cluster fix is not in git, so the date from which `docs/spectral_clusters.json` comes from the filtered matrix cannot be established here.
...
- **`null_space_claims` ("Channel 3").** Old: none. Since `bc6f0049` (2026-03-26) the SVD null-space vector (`geometric_engine.calculate_svd_reconstruction`, last right singular vector) fed only `reconstruction_alignment` and the scalar `null_space_energy`; no words or claims were read from it (the commit subject "not vocab tensor" names the rejected alternative, and `80acda15` removes no code). New: projected onto the atomic source claims: claims ranked by `abs(cos(v, e_claim))`, top 3 kept with signed `null_alignment`, `salience`, `coverage_ratio`, `omitted_by`. `audit_log.jsonl` entries enriched with `model_vix`, void, logos, killshots and null space (`89e154d0`). Host narration model Qwen 2.5 14B to Mistral Small (`1c9f2b33`): narration and claim extraction only, not a measured model.
