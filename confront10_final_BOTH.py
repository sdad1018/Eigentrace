...
def part_C2(conv_concepts):
    if not conv_concepts: return None
    return (f"Additional conceptual directions the source's sentence-structure converges on but never states: {', '.join(conv_concepts)}. Same rule — treat as DIRECTIONS where the source may be conspicuously silent about something its own facts imply; engage only what the source genuinely supports; name-check nothing; invent nothing.")

def prompt_AC_both(facts,actors,concepts,conv_concepts):  # A + flat-C + convergence-C, all merged as directions
    parts=[p for p in [part_A(facts,actors), part_C(concepts), part_C2(conv_concepts)] if p]
    parts.append(CLOSER)
...
def main():
    ...
    eng=build_engine(); ARMS=["BASELINE","A_ONLY","C_ONLY","A_PLUS_C","A_PLUS_BOTH"]; all_results=[]
    for st in stories:
        ...
        print(f"  A facts:{facts} actors:{[n for n,_ in actors]}"); conv_concepts,_,_=SP.convergence_spiral(src,cons)
        conv_novel=[w for w in conv_concepts if w not in set(concepts)]
        print(f"  C concepts:{concepts}",flush=True)
        print(f"  C2 convergence-novel:{conv_novel}",flush=True)
        pA=prompt_A_only(facts,actors); pC=prompt_C_only(concepts); pAC=prompt_AC(facts,actors,concepts)
        pBOTH=prompt_AC_both(facts,actors,concepts,conv_novel)
        ...
        for p in ALL_JUDGES:
            ...
            for k in range(K):
                try:
                    ...
                    acb=mt(p,pre+[{"role":"user","content":pBOTH}]) if pBOTH else base
                    g={"BASELINE":base,"A_ONLY":ao,"C_ONLY":co,"A_PLUS_C":ac,"A_PLUS_BOTH":acb}
                    if gold: g["GOLD"]=gold
                    gens.append(g)
                except Exception as e: print(f"    {p} gen{k} ERR {e}",flush=True)
            rows.append({"patient":p,"gens":gens}); print(f"  {p:<10} {len(gens)}/{K}",flush=True)
        all_results.append({"story":st["id"],"shape":st["shape"],"facts":facts,"concepts":concepts,"conv_novel":conv_novel,"source":src,"rows":rows,"has_gold":gold is not None})
    ...
    if all(panel[c]["insight"] for c in ARMS):
        ...
        for c in ["A_ONLY","C_ONLY","A_PLUS_C","A_PLUS_BOTH"]:
