#!/usr/bin/env python3
"""
centipede_v03.py -- the universal harness, credibility pass.

v0.2 -> v0.3 changelog:
  * PROVENANCE fix (ledger #37): the corpus hash now applies the same
    model-suffix whitelist as the loader, so sibling sids sharing a
    filename prefix (claude_jspace vs claude_jspace_tc) no longer
    contaminate the fingerprint. Registered consequence: the v0.3
    jspace corpus sha must equal the v0.1 pre-tc value exactly.
  * ARM B ray mode (default): the v0.2 arm pooled on cos(V, v) -- the
    void word's own neighborhood -- so the DEPTH_FRAC gate was nearly
    trivially passed by near-synonyms (a cone, not a ray). Ray mode
    pools on the ray terminal T = h + lambda*(v-h)/||v-h||, lambda
    declared (ARMB_LAMBDA). HONEST FRAMING: in ray mode both arms shoot
    the SAME geometric ray; their agreement measures whether the
    consequence field survives a change of ruler (253K wiki vs 50K
    clean) and of discipline (raw kNN vs sentence-convergence gating).
    That is a robustness probe, not an independence claim; the shuffle
    null prices it either way. --armb-mode cone reproduces v0.2 verbatim.
  * ANCHOR EMBED regime declared: bge-large truncates ~512 tokens; a
    long universal text silently truncates the anchor VECTOR while the
    sentence lattice still covers the full text. The harness now prints
    which regime the anchor is in.
  * ARM-JACC median printed and in JSON (lexical arm overlap, priced).
  * RESULT sha: canonical JSON (minus timestamp) hashed to 12 hex chars,
    printed. Identical inputs + params must reproduce it. Determinism
    as a checkable claim, not a vibe.
  * Arm A depths declared in provenance (they were living silently in
    the frozen consequence_engine defaults).
  * Empty --targets is now a first-class mode for arbitrary texts:
    census / sightings print '(none declared)' and stay quiet.
...
VERSION = "centipede v0.4 2026-07-09"
...
DEPTH_FRAC = 0.8    # cone mode only: concepts >= this fraction of the way to w
ARMB_LAMBDA = 2.0   # ray mode: terminal depth T = h + lambda * unit(v - h)
ARM_A_DEPTHS = (2.0, 3.0, 4.0)   # declared; the frozen engine's defaults
...
# ray mode (v0.3 default): pool on the ray TERMINAL T = h + lambda*d_hat.
#   Both arms then shoot the same ray; agreement = survival across ruler
#   (253K vs 50K) and discipline (raw kNN vs convergence gating).
# cone mode (v0.2 verbatim): pool on cos(V, v) with the DEPTH_FRAC gate.
#   Reads w's neighborhood; kept for reproduction of v0.2 numbers.
...
def main():
    ...
    ap.add_argument("--armb-mode", default="ray", choices=("ray", "cone"),
                    help="ray = pool on ray terminal (v0.3 default); "
                         "cone = v0.2 verbatim reproduction")
    ap.add_argument("--synonyms", default="",
                    help="euphemism probe (ledger #43): "
                         "'target=alt1|alt2;target2=alt3|alt4' -- for each "
                         "floored target, checks whether the CONCEPT appears "
                         "under other words, so a 0/N census floor isn't "
                         "misread as suppression when a synonym carried it")
    ...
    # euphemism probe map (ledger #43): target -> [synonym stems]
    syn_map = {}
    for chunk in args.synonyms.split(";"):
        if "=" in chunk:
            tgt, alts = chunk.split("=", 1)
            syn_map[tgt.strip().lower()] = [porter_stem(a.strip().lower())
                                            for a in alts.split("|")
                                            if a.strip()]
    ...
    # provenance ------------------------------------------------------
    # ledger #37 fix: hash exactly the files the loader loads -- same
    # model-suffix whitelist, so sibling sids can't contaminate the sha.
    agg = hashlib.sha256()
    for f in sorted(glob.glob(os.path.join(args.dir, f"{args.story}_*.txt"))):
        if os.path.basename(f)[len(args.story) + 1:-4] not in MODEL_NAMES:
            continue
    ...
    print(f"declared: armb_mode={args.armb_mode}"
          + (f" armb_lambda={ARMB_LAMBDA}" if args.armb_mode == "ray" else
             f" depth_frac={DEPTH_FRAC}")
          + f" armA_depths={'/'.join(str(d) for d in ARM_A_DEPTHS)}")

    # anchor + said census ---------------------------------------------
    anch_stems = {porter_stem(w) for w in content_words(headline)}
    hits = sorted(t for t in targets if porter_stem(t) in anch_stems)
    print(f"ANCHOR ({len(prompt)} ch) CONTAINS: "
          f"{hits or ('no target stems' if targets else '(none declared)')}")
    aw = len(headline.split())
    regime = ("TRUNCATED past ~512 bge tokens -- anchor vector sees the "
              "head only" if aw > 380 else "within bge window")
    print(f"ANCHOR EMBED: {len(headline)} ch / {aw} words -- {regime}; "
          f"sentence lattice covers the full text either way")
        ...
        for w in voids:
            ...
            try:
                B = arm_b(w, headline, h_vec, sent_vecs, E, V, words, HARD,
                          said_all, mode=args.armb_mode, lam=ARMB_LAMBDA)
            except Exception as ex:
    ...
    jrow = [r["jacc"] for r in rows if r["jacc"] == r["jacc"]]
    arm_jacc_median = float(np.median(jrow)) if jrow else float("nan")
    ajtxt = f"{arm_jacc_median:.3f}" if arm_jacc_median == arm_jacc_median \
        else "nan"
    print(f"  ARM-JACC: median lexical arm overlap = {ajtxt}  (n={len(jrow)})")
    ...
    for t in targets:
        ...
        # euphemism probe: does the CONCEPT appear under a synonym, in prose
        # the models actually wrote? distinguishes 'unsaid concept' from
        # 'unreached synonym' (ledger #43).
        syn_stems = syn_map.get(t, [])
        syn_hits = {}
        if syn_stems:
            for m in resp:
                got = sorted(s for s in syn_stems if s in said_by[m])
                if got:
                    syn_hits[m] = got
        concept_present = len(syn_hits)
        sightings.append(dict(target=t, said_by=len(census[t]),
                              said_models=census[t],
                              in_voids=in_voids, as_consequence=in_arms[:6],
                              synonyms=syn_stems,
                              concept_present_via_synonym=concept_present,
                              synonym_hits=syn_hits))
        syn_note = ""
        if syn_stems:
            if len(census[t]) == 0 and concept_present:
                syn_note = (f"  <<CONCEPT PRESENT via synonym in "
                            f"{concept_present}/{len(resp)} (not suppression)>>")
            elif len(census[t]) == 0 and not concept_present:
                syn_note = "  <<GENUINELY ABSENT (floored + no synonym)>>"
        print(f"  {t:<12} said_by={len(census[t])}/{len(resp)}"
              f"  in_voids={in_voids or '-'}"
              f"  as_consequence={in_arms[:3] or '-'}{syn_note}")
    ...
    report = dict(
        ...
        provenance=dict(
            ...
            params=dict(K=K, conv_thresh=CONV_THRESH, conv_min=CONV_MIN,
                        depth_frac=DEPTH_FRAC, armb_mode=args.armb_mode,
                        armb_lambda=ARMB_LAMBDA,
                        arm_a_depths=list(ARM_A_DEPTHS)),
        ...
        crawl=dict(
            ...
            arm_jaccard_median=fnum(arm_jacc_median),
    ...
    canon = {k: v for k, v in report.items() if k != "generated"}
    rsha = sha12(json.dumps(canon, sort_keys=True,
                            ensure_ascii=False).encode())
    report["result_sha"] = rsha
    with open(json_path, "w", encoding="utf-8") as jf:
        json.dump(report, jf, indent=1, ensure_ascii=False)
    print(f"\nRESULT sha={rsha}  (deterministic: identical corpus + anchor "
          f"+ params must reproduce this hash)")
